"""PytSite Authentication and Authorization Plugin Events Handlers
"""
...
def setup():
    ...
    try:
        _validation.rule.NonEmpty(email, 'auth@email_cannot_be_empty').validate()
    ...
    admin_user.first_name = _lang.t('auth@administrator')
    admin_user.nickname = _util.transform_str_2(admin_user.full_name)
    admin_user.roles = [_api.get_role('admin')]
    admin_user.save()
    _api.restore_user()
    _console.print_success(_lang.t('auth@user_has_been_created', {'login': admin_user.login}))
